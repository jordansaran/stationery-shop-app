...
import {currency, Sale} from "@/interfaces/interfaces";
  ...
  return (
    <>
      <TableRow>
        ...
        <TableCell align="center">{Number(row.totalSale).toLocaleString('pt-BR', currency)}</TableCell>
							  ...
							  <TableBody>
								  {row.cart.items.map((item) => (
										<TableRow key={item.id}>
											...
											<TableCell align="center" sx={{border: "none"}}>{item.unitaryPriceCommission != null ? Number(item.unitaryPriceCommission).toLocaleString(`pt-BR`, currency) : "-"}</TableCell>
											<TableCell align="center" sx={{border: "none"}}>{Number(item.totalProduct).toLocaleString('pt-BR', currency)}</TableCell>
											...
											<TableCell align="center" sx={{border: "none"}}>{item.commission != null ? Number(item.commission).toLocaleString(`pt-BR`, currency) : "-"}</TableCell>
										</TableRow>
								  ))}
								</TableBody>
							  <TableFooter>
								  <TableRow key={"values-all-sum"}>
									  ...
									  <TableCell align="center">
										  <Typography variant={"body2"} fontWeight={"bold"} color={"primary"}>
											  {Number(row.cart.totalSale).toLocaleString('pt-BR', currency)}
											</Typography>
										</TableCell>
									  <TableCell align="center"></TableCell>
									  <TableCell align="center">
										  <Typography variant={"body2"} fontWeight={"bold"} color={"primary"}>
											  {Number(row.cart.totalCommission).toLocaleString('pt-BR', currency)}
											</Typography>
										</TableCell>
									</TableRow>
								</TableFooter>
